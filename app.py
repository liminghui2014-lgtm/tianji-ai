...
def generate_opening_chat(chart_data, name, geju_list):
    ...
    basic, geju_text, palace_text = build_chart_summary(chart_data, geju_list)
    today = datetime.now().strftime("%Y年%m月%d日")
    prompt = f"""当前日期：{today}。你是倪海夏。你刚看完 {name} 的紫微斗数命盘。现在用微信语音口吻主动说第一印象。100字内。说1个核心性格底色(引用命宫主星)。抛1个悬念反问。给3个快捷回复。命盘概要：{basic.get('性别','')} | {basic.get('阳历','')} | {basic.get('四柱','')}。命宫: {next((p for p in chart_data.get('命盘',[]) if p['宫位']=='命宫'), {})}。输出格式：[开场白]\n[反问]\n[快捷回复1] | [快捷回复2] | [快捷回复3]"""
    response = client.messages.create(model=API_MODEL_FAST, max_tokens=300, temperature=0.6, thinking={"type":"disabled"}, messages=[{"role":"user","content":prompt}])
    text_blocks = [b.text for b in response.content if hasattr(b,'text') and b.text]
    return "\n".join(text_blocks) if text_blocks else f"{name}，我看完你的盘了。想听听倪师怎么看吗？\n聊聊事业 | 聊聊感情 | 聊聊财运"
...
if st.session_state.page == "landing":
    ...
    with cta:
        if st.button("开启勘探", key="lp_cta_btn", use_container_width=True):
...
if st.session_state.chart_data is not None:
    ...
    if st.session_state.user_id:
        try: remaining, is_vip = check_daily_quota(st.session_state.user_id)
        except Exception: remaining, is_vip = 10, False
    else:
        remaining, is_vip = 10, False

    chat_tab1, chat_tab2 = st.tabs(["对话记录", "与倪师对话"])
    ...
    # ── 倪师的课后校验（V2 反馈）──
    st.markdown("---")
    st.markdown("""<div style=\"background:rgba(196,168,112,0.08);border:1px solid rgba(196,168,112,0.2);border-radius:10px;padding:16px 18px;text-align:center;\"><p style=\"color:#c4a870;font-size:0.9rem;margin:0;\">盘我看过了，刚才聊的这些，跟你目前的真实处境印证得如何？</p></div>""", unsafe_allow_html=True)
    rating = st.radio("印证程度", ["铁口直断，完全命中","有些参考价值，但不全对","驴头不对马嘴"], index=None, key="fb_rating", horizontal=False)
    if rating:
        score = 5 if "铁口" in rating else (3 if "参考" in rating else 1)
        tags = st.multiselect("哪些地方让你有这种感觉？", ["测算准","态度好","像真人","有代入感","逻辑清楚","有幻觉","重复废话","太笼统","口吻不对","其他"], key="fb_tags")
        text = st.text_area("如果觉得有偏差，是哪里不准？", placeholder="比如：感情宫说得不对，我其实是...", key="fb_text")
        if st.button("提交反馈，再获 3 次对话", key="fb_submit", use_container_width=True):
            if st.session_state.user_id and st.session_state.chart_id:
                try:
                    ...
                    ok = save_feedback_v2(st.session_state.user_id, st.session_state.chart_id, score, tags, text)
                    if ok:
                        claim_feedback_reward(st.session_state.user_id, st.session_state.chart_id)
                        st.session_state.chat_bonus += 3
                        st.success("收到你的反馈，再送你 3 次对话。")
                    else:
